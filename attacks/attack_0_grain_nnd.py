...
def attack0(dataset_name, seed, cuda, 
    ...
    log_dir = f"{file_path}/timelogs/{dataset_name}/logtime_grain_nnd_{seed}"
    os.makedirs(os.path.dirname(log_dir), exist_ok=True)
    while True:
        t1 = time.time()
        max_reception_node = get_max_nnd_node_dense(idx_train, 
                                                    idx_avaliable_temp, 
                                                    min_distance,
                                                    distance_aax,
                                                    num_ones,
        ...
        if count >= (20*label_number):
            ...
            with open(log_dir, 'w') as log_file:
                log_file.write(f"GRAIN(NN-D) {dataset_name} {seed} ")
    ...
    with open(f'./node_selection/GRAIN(NN-D)_{dataset_name}_select_nodes_{(20*label_number)}_{seed}.json', 'w') as f:
    ...
    if model_performance:
        ...
        log_file_path = f"{file_path}/{dataset_name}/log_nnd_{seed}.csv"
    ...
    if False:
        ...
        log_file_path = f"{file_path}/gaps/{dataset_name}/log_grainnnd_{seed}.csv"
